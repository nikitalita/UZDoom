...
void ZCCCompiler::MessageV(ZCC_TreeNode *node, const char *txtcolor, const char *msg, va_list argptr)
{
	...
	composed.Format("%s%s, line %d:%d: ", txtcolor, node->SourceName->GetChars(), node->SourceLoc.Line, node->SourceLoc.Column);
...
FxExpression *ZCCCompiler::ConvertAST(PContainerType *cls, ZCC_TreeNode *ast)
{
	...
	if (ast->NodeType == AST_ExprFuncCall)
	{
		...
		FScriptPosition pos = FScriptPosition(*ast->SourceName, ast->SourceLump, ast->SourceLoc.EndLine, ast->SourceLoc.EndColumn);
